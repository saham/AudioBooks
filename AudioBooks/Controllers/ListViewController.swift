...
extension ListViewController: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return model.podcasts?.count ?? 0
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! ListTableViewCell
        cell.config(podcast: model.podcasts?[indexPath.row])
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let DVC = sb.instantiateViewController(withIdentifier: "DetailsVC") as? DetailsViewController{
            DVC.podcast =  model.podcasts?[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if let count = model.podcasts?.count, count - 1 == indexPath.row {
            setupActivity(isRunning:  true)
            guard !API.isPaging else {return}
            API.fetchData {[weak self] res in
                switch res {
                case .success(let listing):
                    if let newListing = listing.podcasts {
                        self?.model.podcasts?.append(contentsOf: newListing)
